...
function referenceAnalysisPrompt(): string {
  return [
    ...
    "Map visible base coverage independently for forehead, temples, nose, cheeks, and chin/jaw as none, sheer, medium, or full.",
    "Set baseCoverage.expectedContinuity to full-face when foundation or tone/finish treatment visibly continues across the main face regions, localized only when coverage is intentionally limited, and none when no base is visible.",
    "Base coverage means an intentional tone, opacity, or finish change; it does not require hiding pores, moles, freckles, or natural texture.",
    ...
    "Do not put routine brows, mascara, blush, contour, or base details in mustMatch unless they are genuinely dominant, but always complete the required baseCoverage map.",
...
function qualityReviewPrompt(spec: MakeupReferenceSpec): string {
  return [
    ...
    "Score baseCoverageContinuityScore independently by comparing the reference base coverage map with the generated result across forehead, temples, nose, cheeks, and chin/jaw.",
    "When expected continuity is full-face, list every visibly untreated or discontinuous required zone in baseCoverageMissing. A made-up center face with a bare forehead is a blocking failure even if eyes, cheeks, and lips match.",
    "When expected continuity is localized or none, baseCoverageMissing should be empty unless the generated result contradicts that intended placement; use a high continuity score when it correctly matches the absence or localization of base.",
    "Preserving pores, moles, freckles, and natural texture is compatible with continuous foundation coverage and must not be mistaken for an untreated zone.",
    ...
    "Supporting brow, cheek, contour, or highlight differences may affect overallScore but must not dominate makeupSimilarityScore or become criticalMissing unless explicitly listed as highest-priority.",
    "Base coverage continuity is always evaluated through baseCoverageContinuityScore and baseCoverageMissing, regardless of whether base is a focal feature.",
