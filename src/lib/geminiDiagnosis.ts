...
function diagnosisPrompt(options: {
  ...
  return [
    ...
    "Make makeupPlan concrete and zone-based for base, brows, eyes, cheeks, lips, contour and highlight. Each array item must be an actionable instruction about product type, placement, technique, or avoid-point; do not repeat visual evidence as the main content.",
    "Do not mix makeup product categories. Base products such as foundation, concealer, powder, or setting powder must only appear in makeupPlan.base or colorPalette entries with usage 'base'. Eye products such as eyeshadow, eyeliner, mascara, or lashes must only appear in eye-related advice. Cheeks must focus on blush placement/color; lips must focus on lip color/finish.",
    "For colorPalette.recommended, each name must match its usage: base for base products, eyes for eye colors/products, cheeks for blush colors, lips for lip colors, multi only for true cross-face color families such as rose, peach, mauve, beige, tea, nude, or brown. Do not label foundation, concealer, powder, eyeliner, mascara, or eyeshadow as multi.",
    "Return exactly four scenarioStrategies using scenario values work, date, camera, and daily. Each scenario must explain colors, technique, what to avoid, and how the user can validate the result in real lighting. scenarioStrategies.colors should list only accent color families for eyes, cheeks, or lips, not foundation, concealer, powder, or other base products.",
